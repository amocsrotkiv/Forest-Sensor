...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt)
{
    ...
    if (p_evt_write->handle == p_cus->custom_value_handles.value_handle)
    {
		...
		ble_gatts_value_t rx_data; //struktúra az adat fogadáshoz
	...
	if(*p_evt_write->data == 0x01)
		...
		uint32_t err_code = NRF_SUCCESS;
		/*somehow getting the current length of the characteristic written to before*/
		uint8_t lenbuff[1];
		ble_gatts_value_t len_get; //probe buffer
		memset(&len_get, 0, sizeof(len_get)); //triv.
		
		len_get.len=0;//we are reading more than 0 byte of data so due to the documentation we will get back the actuel lenght of the charecteristic
		len_get.p_value = &lenbuff[0]; //pointing to the current buffer
		len_get.offset=0;//offset 0
		err_code = sd_ble_gatts_value_get(p_cus->conn_handle, p_cus->viktor_value_handles.value_handle,&len_get);// itt visszakapjuk a len_get,len mezőjében hogy  mekkora buffer kell az adathoz
		/* by this point we know the lenght of the charateristic we are trying to read*/
		uint8_t * buffer; //place the data to this buffer
		buffer=nrf_malloc(len_get.len*sizeof(uint8_t));// allocating space 
		ble_gatts_value_t rx_data; 
		memset(&rx_data, 0, sizeof(rx_data)); //triv.
		
		rx_data.len=len_get.len;//20 byteo of data
		rx_data.p_value = buffer; //rámutatunk a cél bufferre
		rx_data.offset=0;//offset 0
		err_code = sd_ble_gatts_value_get(p_cus->conn_handle, p_cus->viktor_value_handles.value_handle,&rx_data);//getting the vlaue of the characteristic	
		//copying the just got data to this buffer "static char sendbuffer[160]"
		for(i=combinedsize;i<rx_data.len+combinedsize;i++)
		{
			sendbuffer[i]=buffer[i-combinedsize];
		}
		combinedsize+=rx_data.len;
		...
		err_code = sd_ble_gatts_value_get(p_cus->conn_handle, p_cus->custom_value_handles.value_handle,&rx_data);// a custom valueból megszerezzük az adatot
		for(i=0;i<20;i++){
			sendbuffer[(20*writenumber)+i]=buffer[i];
			
		}
		NRF_LOG_INFO("%d writenumber:\r\n", writenumber); 
		for(i=0; i<=160; i++)
		{
		NRF_LOG_INFO("%s\n", sendbuffer[i]);
		}
