...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt)
{
	...
	if(*p_evt_write->data == 0x01)
		...
		uint8_t lenbuff[0];
		ble_gatts_value_t len_get; //próbabuff a hossz megviszgálásához
		memset(&len_get, 0, sizeof(len_get)); //triv.
		
		len_get.len=20;//20 byteos adatot olvasunk
		len_get.p_value = &lenbuff[0]; //rámutatunk a cél bufferre ami 0 byteos
		...
		char * buffer; //ebbe a bufferbe várjuk a 20 byteot
		buffer=nrf_malloc(len_get.len*sizeof(char));// elegendő helyet foglalunk a bufferünknek
		ble_gatts_value_t rx_data; //struktúra az adat fogadásához
		memset(&rx_data, 0, sizeof(rx_data)); //triv.
		...
		//Feladat_Belekéne másolni a static char buffer-be a most megkapott adatot
		for(i=0;i<len_get.len;i++)
		{
			sendbuffer[i+combinedsize]=buffer[i];
		}
		combinedsize=combinedsize+len_get.len;
		
		nrf_free(buffer);
		//160 adatnál nem küldhet el többet egyszerre
		if(160-combinedsize<21){
			combinedsize=0;
		}
